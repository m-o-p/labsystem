...
class UserForm(Form):
    username = StringField('Username', [validators.required(), validators.length(min=4, max=20)])
    name = StringField('Full Name', [validators.required(), validators.length(min=4, max=80)])
