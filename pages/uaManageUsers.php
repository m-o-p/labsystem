...
// DB Query
    $result = $userDBC->mkSelect( '*',
                                  ...
                                  ( (isset($_POST['restrictTo'])) ? // POST overrides GET!
                                      ( $_POST['restrictTo']!='' ? $_POST['restrictTo'].'=1'.( !empty($searchFor)?' AND ':'' ) : '' ) : // empty? ignore!
                                    ( ($GLOBALS['url']->available('restrictTo')) && $GLOBALS['url']->get('restrictTo')!='' ? $GLOBALS['url']->get('restrictTo').'=1'.( !empty($searchFor)?' AND ':'' ) : '' )
                                   ).
                                   ( !empty($searchFor) ? '('.$cfg->get('UserDBField_name').' LIKE "%'.$userDBC->escapeString($searchFor).'%" OR '.$cfg->get('UserDBField_forename').' LIKE "%'.$userDBC->escapeString($searchFor).'%")' : ''),
...
// form
    ...
    while( $data = mysql_fetch_assoc( $result ) ){
      ...
      $pge->put('<select style="float:left;margin-right:2em" multiple="multiple" tabindex="'.$pge->nextTab++.'" name="'.$data[ $cfg->get('UserDBField_uid') ].'_multi[]">');
      $courseList = '';
